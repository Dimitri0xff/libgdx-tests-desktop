...
public class FreeTypeIncrementalTest extends GdxTest {
	...
	public void create() {
		...
		// By default latin chars are used for x and cap height, causing some
		// fonts to display non-latin chars out of bounds.
		// FIXME
		data.xChars = new char[] { 'a' };
		data.capChars = new char[] { 'a' };
	...
	static public class SimplifiedChinese {
		public static int getWrapIndex(Array<Glyph> glyphs, int start) {
			...
			for (; i >= 1; i--) {
				...
				if (startChar < 127 && endChar < 127 && !Character.isWhitespace(startChar))
					continue;
				return i;
			}
			return start;
		...
		static private boolean legalAtStart(int ch) {
			...
			case 'a':
				/*
				 * case 'Â¨': case 'Â°': case 'Â·': case 'Ë‡': case 'Ë‰': case
				 * 'â€•': case 'â€–': case 'â€™': case 'â€�': case 'â€ž': case
				 * 'â€Ÿ': case 'â€ ': case 'â€¡': case 'â€º': case 'â„ƒ': case
				 * 'âˆ¶': case 'ã€�': case 'ã€‚': case 'ã€ƒ': case 'ã€†': case
				 * 'ã€ˆ': case 'ã€Š': case 'ã€Œ': case 'ã€Ž': case 'ã€•': case
				 * 'ã€—': case 'ã€ž': case 'ï¹˜': case 'ï¹š': case 'ï¹œ': case
				 * 'ï¼�': case 'ï¼‚': case 'ï¼…': case 'ï¼‡': case 'ï¼‰': case
				 * 'ï¼Œ': case 'ï¼Ž': case 'ï¼š': case 'ï¼›': case 'ï¼Ÿ': case
				 * 'ï¼½': case 'ï½€': case 'ï½œ': case 'ï½�': case 'ï½ž':
				 */
				return false;
			}
			return true;
		...
		static private boolean legalAtEnd(int ch) {
			...
			case 'c':
				/*
				 * case 'â€˜': case 'â€œ': case 'ã€ˆ': case 'ã€Š': case 'ã€Œ':
				 * case 'ã€Ž': case 'ã€�': case 'ã€”': case 'ã€–': case 'ã€�':
				 * case 'ï¹—': case 'ï¹™': case 'ï¹›': case 'ï¼„': case 'ï¼ˆ':
				 * case 'ï¼Ž': case 'ï¼»': case 'ï½›': case 'ï¿¡': case 'ï¿¥':
				 */
				return false;
			}
			return true;
