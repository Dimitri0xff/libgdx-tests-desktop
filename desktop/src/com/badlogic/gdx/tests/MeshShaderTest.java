...
public class MeshShaderTest extends GdxTest {
	...
	public void create() {
		...
		meshCustomVA.setVertices(new float[] { -0.5f, -0.5f, 0, FLOAT_WHITE, toSingleFloat(0, 1), 0.5f, -0.5f, 0,
				FLOAT_WHITE, toSingleFloat(1, 1), 0.5f, 0.5f, 0, FLOAT_WHITE, toSingleFloat(1, 0), -0.5f, 0.5f, 0,
				FLOAT_WHITE, toSingleFloat(0, 0) });
	...
	private static float toSingleFloat(float u, float v) {
		...
		return NumberUtils.intToFloatColor(vu); // v will lose some precision
												// due to masking
	}
